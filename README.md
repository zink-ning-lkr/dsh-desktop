...
| 命令栏 ☀ 按钮 / Ctrl+K 命令面板 | 主题三态循环(跟随系统 / 深色 / 浅色);全部自绘窗口即时换肤(Win11 下辅助窗为 Mica/Acrylic 系统材质) |
| ☰ 菜单 → 设置… | 三分区设置窗:更新与下载(分段数 2-16 滑杆 / 镜像源 / npm 换源指引)、外观与行为(主题三态 / 关闭行为 / 自动打开浏览器)、高级(路径 / 内存);菜单与命令面板深链直达各分区 |
| 任务中心 | 检查更新/下载/安装多任务并存列表:进度互不顶掉、行级取消、完成项折叠历史(可一键清空)、「后台」挂后台完成时桌面通知 |
...
| `downloadSegments` | 桌面端更新下载的分段数(2-16,默认 6;由设置窗「更新与下载」分区管理) |
...
├── i18n.js + i18n/        # 文案集中与术语统一:t(key, params) + zh-CN.json(全部自绘页面已接入)
...
├── settings.html + settings-preload.js    # 设置窗(三分区):更新与下载/外观与行为/高级;菜单与命令面板深链直达
├── palette.html + palette-preload.js      # 命令面板(Ctrl+K):全量命令搜索、最近使用、分类标签、键盘导航
├── toast.html + toast-preload.js          # 通知宿主:右上角瞬时提示,点击穿透不抢焦点,悬停延长驻留
...
- **国内网络检查/下载慢**:设置窗「更新与下载」分区配置镜像源与分段数;dsh 本体更新可复制窗口内的 npm 换源命令执行
